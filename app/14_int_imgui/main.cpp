...
class Gui : public GameObject {
 public:
  ...
  void OnPostStart() override {
    _pass = GetApp().GetRenderPass<ColorPass>("Color Pass");
  }

  void OnGui() override {
    ImGui::Begin("Sphere Property", &_canShow);
    if (_firstCall) {
      ImGui::SetWindowSize({250, 100});
      _firstCall = false;
    }
    ImGui::SliderFloat("metallic", &(_pass->metallic), 0.0f, 1.0f);
    ImGui::End();
  }

  bool _firstCall{true};
  bool _canShow{true};
  std::shared_ptr<ColorPass> _pass;
};
